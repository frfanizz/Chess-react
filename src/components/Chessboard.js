...
function getMoveChessNotation(initRow, initCol, endRow, endCol, isCapture, _promoteToPiece) {
...
function getPieceValidMoves(board, row, col, colorIsWhite) {
	...
	if (square !== null) {
		if (square.isWhite === colorIsWhite) {
			...
			switch (square.piece) {
				case pawnPiece():
					validMovesIgnoreCheck = getPawnValidMoves(board, row, col, colorIsWhite);
					break;
				case rookPiece():
					validMovesIgnoreCheck =  getRookValidMoves(board, row, col, colorIsWhite);
					break;
				case knightPiece():
					validMovesIgnoreCheck =  getKnightValidMoves(board, row, col, colorIsWhite);
					break;
				case bishopPiece():
					validMovesIgnoreCheck =  getBishopValidMoves(board, row, col, colorIsWhite);
					break;
				case queenPiece():
					validMovesIgnoreCheck =  getQueenValidMoves(board, row, col, colorIsWhite);
					break;
				case kingPiece():
					validMovesIgnoreCheck =  getKingValidMoves(board, row, col, colorIsWhite);
					break;
			}
		}
	}
...
function getPawnValidMoves(board, row, col, colorIsWhite) {
	...
	if (canCheckLeft
		...
		&& (board.boardArr[row + moveForward(1, colorIsWhite)][col-1].isWhite !== colorIsWhite)) {
		validMoves.push([row + moveForward(1, colorIsWhite), col-1]);
	}
	// check right capture
	if (canCheckRight
...
function getKnightValidMoves(board, row, col, colorIsWhite) {
	...
	[llu, luu, ruu, rru, rrd, rdd, ldd, lld].forEach((move) => {
		if (move.cRow >= 0 && move.cCol >= 0 && move.cRow < 8 && move.cCol < 8 && (
			board.boardArr[move.cRow][move.cCol] === null || board.boardArr[move.cRow][move.cCol].isWhite !== colorIsWhite)) {
...
function getKingValidMoves(board, row, col, colorIsWhite, isInCheck) {
	...
	if (!isInCheck && !board.boardArr[row][col].hasMoved) {
		if (board.boardArr[row][0] !== null
			&& !board.boardArr[row][0].hasMoved) {
			if (board.boardArr[row][1] === null
		...
		if (board.boardArr[row][7] !== null
			&& !board.boardArr[row][7].hasMoved) {
			if (board.boardArr[row][5] === null
				&& board.boardArr[row][6] === null
				) {
					const boardClone = cloneBoard(board);
...
function canCheckLambda(board, currRow, currCol, currIsWhite, currRowUpdate, currColUpdate, piecesToCheck) {
...
function checkIfInCheck(board, checkWhite) {
	...
	if (canBishopOrQueenTakeKingAtPos(board, kingLoc[0], kingLoc[1], checkWhite)) {
		return true;
	}
...
function getValidMovesLambda(board, currRow, currCol, currIsWhite, currRowUpdate, currColUpdate) {
	let validMoves = [];
	while(currRow >= 0 && currCol >= 0 && currRow < 8 && currCol < 8) {
		if (board.boardArr[currRow][currCol] === null) {
...
function getRookValidMoves(board, row, col, colorIsWhite) {
	let validMoves = [];
	// check left
	const validMovesLeft = getValidMovesLambda(board, row, col - 1, colorIsWhite, (r) => { return r; }, (c) => { return c - 1; });
	validMoves = validMoves.concat(validMovesLeft);
	// check right
	const validMovesRight = getValidMovesLambda(board, row, col + 1, colorIsWhite, (r) => { return r; }, (c) => { return c + 1; })
	validMoves = validMoves.concat(validMovesRight);
	// check down
	const validMovesDown = getValidMovesLambda(board, row + 1, col, colorIsWhite, (r) => { return r + 1; }, (c) => { return c; });
	validMoves = validMoves.concat(validMovesDown);
	// check up
	const validMovesUp = getValidMovesLambda(board, row - 1, col, colorIsWhite, (r) => { return r - 1; }, (c) => { return c; });
...
function getBishopValidMoves(board, row, col, colorIsWhite) {
	let validMoves = [];
	// check up-left
	const validMovesUpLeft = getValidMovesLambda(board, row - 1, col - 1, colorIsWhite, (r) => { return r - 1; }, (c) => { return c - 1; });
	validMoves = validMoves.concat(validMovesUpLeft);
	// check up-right
	const validMovesUpRight = getValidMovesLambda(board, row - 1, col + 1, colorIsWhite, (r) => { return r - 1; }, (c) => { return c + 1; });
	validMoves = validMoves.concat(validMovesUpRight);
	// check down-left
	const validMovesDownLeft = getValidMovesLambda(board, row + 1, col - 1, colorIsWhite, (r) => { return r + 1; }, (c) => { return c - 1; });
	validMoves = validMoves.concat(validMovesDownLeft);
	// check down-right
	const validMovesDownRight = getValidMovesLambda(board, row + 1, col + 1, colorIsWhite, (r) => { return r + 1; }, (c) => { return c + 1; });
	validMoves = validMoves.concat(validMovesDownRight);
	return validMoves;
}
function getQueenValidMoves(board, row, col, colorIsWhite) {
	return getRookValidMoves(board, row, col, colorIsWhite).concat(getBishopValidMoves(board, row, col, colorIsWhite));
}
...
function getAllPossibleMoves(board, isWhiteTurn) {
	...
	for (const key in board.pieceIndex.white) {
		...
		const validMoves = getPieceValidMoves(board, piece[0], piece[1], isWhiteTurn);
...
class Chessboard extends React.Component {
  constructor(props) {
	...
	movePiece(initRow, initCol, endRow, endCol) {
		...
		if ((endRow === 0 || endRow === 7) && isPiecePawn(this.state.board.boardArr[initRow][initCol])) {
			isPromotion = true;
		}
		const { isCapture, isCastling } = makeMoveOnBoard(boardClone, initRow, initCol, endRow, endCol);
		...
		const possibleMovesForSelectedClone = this.state.possibleMovesForSelected.slice();
		...
		let promotionPreviousPositionClone = this.state.promotionPreviousPosition.slice();
		if (isPromotion) {
			...
			allPossibleMovesClone = getAllPossibleMoves(boardClone, !this.state.isWhiteTurn);
		}
		const gameOverFlagsClone = getGameOverFlags(isNextInCheck, allPossibleMovesClone.count);
		this.setState(
			{
				...this.state,
				board: boardClone,
	...
	promotePawn(promoteToPiece) {
		...
		moveHistoryClone.push(getMoveChessNotation(initRow, initCol, endRow, endCol, isCapture, false, promoteToPiece));
		const boardHistoryClone = this.state.boardHistory.slice();
		boardHistoryClone.push(boardClone.boardArr);
		const isNextInCheck = checkIfInCheck(boardClone, !this.state.isWhiteTurn);
		const allPossibleMovesClone = getAllPossibleMoves(boardClone, !this.state.isWhiteTurn);
		const possibleMovesForSelectedClone = this.state.possibleMovesForSelected.slice();
		possibleMovesForSelectedClone.forEach((row) => {
			row.forEach((_, colIdx) => {
				row[colIdx] = false;
	...
	moveOrGetValidMoves(row, col) {
		...
		// Selecting a piece > get all possible moves of that piece
			...
			if (this.state.board.boardArr[row][col].isWhite !== this.state.isWhiteTurn) {
				...
				const validMoves = this.state.allPossibleMoves[this.state.board.boardArr[row][col].id];
